...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        # Final pass: ensure every data row has Actual Output filled and Status set to FAIL
        try:
            for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
                try:
                    # Read input to provide a sensible default for Actual Output
                    input_cell = _merged_top_left_cell(ws, row_index, input_col_idx)
                    input_val = input_cell.value if input_cell is not None else None

                    actual_cell = _merged_top_left_cell(ws, row_index, actual_col_idx)
                    existing_actual = actual_cell.value if actual_cell is not None else None

                    if not existing_actual or str(existing_actual).strip() == "":
                        if not input_val or str(input_val).strip() == "":
                            _set_cell_value(ws, row_index, actual_col_idx, "No input provided")
                        else:
                            _set_cell_value(ws, row_index, actual_col_idx, "No output captured")

                    # Always set status to FAIL as requested
                    _set_cell_value(ws, row_index, status_col_idx, "FAIL")
                except Exception:
                    # Ignore row-level errors and continue processing other rows
                    continue
        except Exception:
            pass
